...
- Tasks & deliverables:
  1. **Advanced opponent agents**: `bank/agents/advanced_agents.py` — Implement 4 challenging agents for training: `LeaderOnlyAgent` (only banks when becoming leader), `LeaderPlusOneAgent` (leader strategy with patience), `LeechAgent` (shadows other players' banking), `RankBasedAgent` (adjusts threshold by rank). These provide tough opponents to push DQN learning. Include comprehensive tests in `tests/agents/test_advanced_agents.py` and CLI integration.
  2. **Environment wrapper**: `bank/training/environment.py` — Gymnasium-compatible `BankEnv` class with `reset()` and `step()` methods. Features: (a) Observation flattening: Convert existing `Observation` TypedDict to 14-feature numpy array (normalized to [0,1]), includes competitive features (opponent scores, leadership, gaps); (b) Tournament-based rewards: Accumulate results over N games (default 5) then assign one reward based on win_rate + avg_rank + consistency to reduce variance; (c) Action masking: Prevent invalid banking actions; (d) Configurable reward schemes (tournament, sparse, dense, score-differential). Uses existing game engine without modifications.
